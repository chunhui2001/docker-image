...
const router = express.Router({ caseSensitive: app.get('case sensitive routing'), strict: app.get('strict routing') });
...
function skipLog (req: express.Request, res: express.Response) {
    ...
    if(url.toLowerCase().trim() === '/info' || url.toLowerCase().trim() === '/info/') {
        return true;
    }
    if(url.indexOf('?')>0)
